            ...
            <div class="card">
                ...
                <div class="card-body">
                    <x-form route="service.update" updateId="{{ $service->id }}" type="Submit" method="true">
                        ...
                        <label>{{ __('Name') }}</label>
                        <x-input name="name" type="text" placeholder="{{ __('Service') . ' ' . __('Name') }}" value="{{ $service->name }}" />
                        ...
                        <x-select :multi="true" name="variant_ids[]">
                            ...
                            <option value="{{ $variant->id }}" {{ in_array($variant->id, $service->variants->pluck('id')->toArray()) ? 'selected' : '' }}>
                                {{ $variant->name }}
                            </option>
                            @endforeach
                        </x-select>
                        ...
                        <x-textarea name="description" placeholder="{{ __('Service') . ' ' . __('Description') }}" value="{{ $service->description }}" />

                        <label>{{ __('Additional_Service') }}</label>
                        <x-select :multi="true" name="additional[]">
                            @foreach ($additionals as $additional)
                            <option value="{{ $additional->id }}" {{ in_array($additional->id, $service->additionals->pluck('id')->toArray()) ? 'selected' : '' }}>
                                {{ $additional->title }}
                                {{ $additional->title }}
                            </option>
                            @endforeach
